...
/*   Updated: 2022/12/08 15:03:57 by kcatrix          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	if (open(s->pathspritee, O_DIRECTORY) != -1)
		exit(printf("erreur chemin E est un dossier\n"));
	if (open(s->pathspriten, O_DIRECTORY) != -1)
		exit(printf("erreur chemin N est un dossier\n"));
	if (open(s->pathsprites, O_DIRECTORY) != -1)
		exit(printf("erreur chemin S est un dossier\n"));
	if (open(s->pathspritew, O_DIRECTORY) != -1)
